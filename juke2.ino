...
void continuePlayingLong()
{
    if (longPressed)
    {
        ...
        if (busyPinState == 1) // has it gone from low to high?, meaning the track finished
        {
            ...
            lcd.print(" * Random Play * ");
            lcd.setCursor(0, 1);
            lcd.print("Current Selection");
            lcd.setCursor(16, 2);
            // lcd.print(randomNumber[playingIndex]);
            if (keypadLong)
                lcd.print(randomNumber[playingIndex]);
            else
                lcd.print(convertToUpperCase(intToAlphanumeric(randomNumber[playingIndex])));
            lcd.setCursor(0, 3);
            lcd.print("Reset = Stop play");
            playingIndex++;
        }
    }
}
